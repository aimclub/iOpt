...
class FunctionConsoleFullOutput:
    ...
    def printIterInfo(self, savedNewPoints: List[SearchDataItem], mode: int) -> None:
    ...
    def printFinalResult(self, solution: Solution, status: bool) -> None:
...
class ConsoleOutputer:
    def printInit(self, eps: float, r: float, epsR: float, itersLimit: int, floatdim: int,
                  numberOfObjectives: int, numberOfConstraints: int,
                  lowerBoundOfFloatVariables: List[float], upperBoundOfFloatVariables: List[float]) -> None:
        dim = floatdim
        print()
        print("-" * (30 + 20 * dim + 2))
    ...
    def printIter(self, point: List[float], value: float, iter: int, mode: int) -> None:
        # time.sleep(0.1)
        dim = len(point)
        if mode == 1:
            print("|", end=' ')
    ...
    def printResult(self, solved: bool, numberOfGlobalTrials: int, numberOfLocalTrials: int,
                    solvingTime: float, solutionAccuracy: float,
                    bestTrialPoint: List[float], bestTrialValue: float) -> None:

...
class GKLSRandomGenerator:
    ...
    @staticmethod
    def mod_sum(x: float, y: float) -> float:
        return (((x) + (y)) - (int)((x) + (y)))   # (x+y) mod 1.0 */

    @staticmethod
    def is_odd(s: int) -> int:
        return ((s) & 1)
    ...
    def Initialize(self, seed: int,
                   rnd_num_mem: npt.NDArray[np.double],
                   rand_condition_mem: npt.NDArray[np.double]) -> None:
        ...
        while (t > 0):
            ...
            for j in range(GKLSRandomGenerator.KK + GKLSRandomGenerator.KK - 2, GKLSRandomGenerator.KK - 1, -1):
                if (ul[j] != 0):
                    ul[j - (GKLSRandomGenerator.KK - GKLSRandomGenerator.LL)] = ulp - \
                        ul[j - (GKLSRandomGenerator.KK - GKLSRandomGenerator.LL)]
                    u[j - (GKLSRandomGenerator.KK - GKLSRandomGenerator.LL)
                      ] = GKLSRandomGenerator.mod_sum(u[j - (GKLSRandomGenerator.KK - GKLSRandomGenerator.LL)], u[j])
    ...
    def GenerateNextNumbers(self) -> None:
        i = 0
        j = 0
        ...
        for j in range(GKLSRandomGenerator.KK, n):
            self.rnd_num[j] = GKLSRandomGenerator.mod_sum(
                self.rnd_num[j - GKLSRandomGenerator.KK], self.rnd_num[j - GKLSRandomGenerator.LL])
        j = j + 1
        ...
        for i in range(GKLSRandomGenerator.LL, GKLSRandomGenerator.KK):
            self.ran_u[i] = GKLSRandomGenerator.mod_sum(
                self.rnd_num[j - GKLSRandomGenerator.KK], self.ran_u[i - GKLSRandomGenerator.LL])
            j = j + 1

    def GetRandomNumber(self, indx: int) -> float:
        return self.rnd_num[indx]

    def GetGeneratorState(self, indx: int) -> float:

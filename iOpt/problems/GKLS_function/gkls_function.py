...
class T_GKLS_Minima:
    def __init__(self) -> None:
        ...
        self.local_min = np.zeros((1, 1), dtype=np.double)
        self.rho = np.zeros(1, dtype=np.double)
...
class GKLSClass(Enum):
    Hard = 0
    Simple = 1


class GKLSFuncionType(Enum):
    TND = 0
    TD = 1
    TD2 = 2
...
class GKLSParameters:
    def __init__(self,
                 _dimension: int,
                 _globalMinimumValue: float,
                 _numberOfLocalMinima: int,
                 _globalDistance: float,
                 _globalRadius: float,
                 _GKLStype: int) -> None:
...
class GKLSFunction:
    ...
    def __init__(self) -> None:
        ...
        self.GKLS_global_dist = 0.0  # distance from the paraboloid minimizer *#
        # to the global minimizer *#
        self.GKLS_global_radius = 0.0  # radius of the global minimizer *#
        # attraction region *#
        self.GKLS_global_value = 0.0  # global minimum value, *#
    ...
    def Calculate(self, x: npt.NDArray[np.double]) -> float:
    ...
    def GKLS_domain_free(self) -> None:
        self.mIsDomainMemeoryAllocated = False

    def GKLS_free(self) -> None:
        self.isArgSet = 0
        self.mIsGeneratorMemoryAllocated = False

    def CalculateDFunction(self, x: npt.NDArray[np.double]) -> float:
        i = 0
        index = 0
        norm = 0.0
        scal = 0.0
        a = 0.0
        rho = 0.0  # * working variables *#

        if not self.isArgSet:
            return GKLSFunction.GKLS_MAX_VALUE

        for i in range(self.GKLS_dim):
            if ((x[i] < self.GKLS_domain_left[i] - GKLSFunction.GKLS_PRECISION)
                    or (x[i] > self.GKLS_domain_right[i] + GKLSFunction.GKLS_PRECISION)):
                return GKLSFunction.GKLS_MAX_VALUE
    # Check wether x belongs to some basin of local minima, M(index) <> T *#
    # Attention: number of local minima must be >= 2 *#
        index = 1
        while ((index < self.GKLS_num_minima) and (self.GKLS_norm(
                self.GKLS_minima.local_min[index], x) > self.GKLS_minima.rho[index])):
            index = index + 1
    ...
    def GKLS_norm(self, x1: npt.NDArray[np.double], x2: npt.NDArray[np.double]) -> float:
        i = 0
        norm = np.double(0)
        for i in range(self.GKLS_dim):
    ...
    def SetFunctionClass(self, _GKLStype: GKLSClass, classDimension: int) -> None:
    ...
    def SetFunctionNumber(self, number: int) -> None:
        self.mFunctionNumber = number
        if self.mIsGeneratorMemoryAllocated:
            self.GKLS_free()
        self.GKLS_arg_generate(number)

    def GKLS_arg_generate(self, nf: int) -> int:
        i = 0
        j = 0
        error = 0
        sin_phi = 0.0  # for generating of the global minimizer coordinates #
    ...
    # the paraboloid minimum value #
        for i in range(self.GKLS_dim):
            self.GKLS_minima.local_min[0][i] = self.GKLS_domain_left[i] + \
                self.rnd_num[self.rnd_counter] * (self.GKLS_domain_right[i] - self.GKLS_domain_left[i])
            self.rnd_counter += 1
        ...
        self.GKLS_minima.local_min[1][0] = self.GKLS_minima.local_min[0][0] + \
            self.GKLS_global_dist * np.cos(GKLSFunction.PI * self.rnd_num[self.rnd_counter])
        if ((self.GKLS_minima.local_min[1][0] > self.GKLS_domain_right[0] - GKLSFunction.GKLS_PRECISION)
                or (self.GKLS_minima.local_min[1][0] < self.GKLS_domain_left[0] + GKLSFunction.GKLS_PRECISION)):
            self.GKLS_minima.local_min[1][0] = self.GKLS_minima.local_min[0][0] - \
                self.GKLS_global_dist * np.cos(GKLSFunction.PI * self.rnd_num[self.rnd_counter])
    ...
    # the coordinates x(i), i=1,...,GKLS_dim-2 (not last!) #
        for j in range(1, self.GKLS_dim - 1):
            self.GKLS_minima.local_min[1][j] = self.GKLS_minima.local_min[0][j] + self.GKLS_global_dist * \
                np.cos(2.0 * GKLSFunction.PI * self.rnd_num[self.rnd_counter]) * sin_phi
            if ((self.GKLS_minima.local_min[1][j] > self.GKLS_domain_right[j] - GKLSFunction.GKLS_PRECISION)
                    or (self.GKLS_minima.local_min[1][j] < self.GKLS_domain_left[j] + GKLSFunction.GKLS_PRECISION)):
                self.GKLS_minima.local_min[1][j] = self.GKLS_minima.local_min[0][j] - self.GKLS_global_dist * \
                    np.cos(2.0 * GKLSFunction.PI * self.rnd_num[self.rnd_counter]) * sin_phi
            ...
            while (i < self.GKLS_num_minima):
                while (True):
                    ...
                    for j in range(self.GKLS_dim):
                        self.GKLS_minima.local_min[i][j] = self.GKLS_domain_left[j] + \
                            self.rnd_num[self.rnd_counter] * (self.GKLS_domain_right[j] - self.GKLS_domain_left[j])
                        self.rnd_counter += 1
                        if (self.rnd_counter == GKLSRandomGenerator.NUM_RND):
                            self.mRndGenerator.GenerateNextNumbers()  # ranf_array(rnd_num, NUM_RND);
                            self.rnd_counter = 0
                    ...
                    if not ((self.GKLS_global_radius + gap) -
                            self.GKLS_norm(self.GKLS_minima.local_min[i], self.GKLS_minima.local_min[1]) >
    ...
    def GKLS_parameters_check(self) -> int:
        ...
        for i in range(self.GKLS_dim):
            ...
            if (tmp < min_side):
                min_side = tmp
        if ((self.GKLS_global_dist >= 0.5 * min_side - GKLSFunction.GKLS_PRECISION)
                or (self.GKLS_global_dist <= GKLSFunction.GKLS_PRECISION)):
            return GKLSFunction.GKLS_GLOBAL_DIST_ERROR  # global distance error #
        if ((self.GKLS_global_radius >= 0.5 * self.GKLS_global_dist + GKLSFunction.GKLS_PRECISION)
    ...
    def GKLS_initialize_rnd(self, dim: int, nmin: int, nf: int) -> int:
    ...
    def GKLS_coincidence_check(self) -> int:
    ...
    def GKLS_set_basins(self) -> int:

        i = 0
        j = 0
        temp_min = 0.0  # temporary  #
        temp_d1 = 0.0
        temp_d2 = 0.0  # variables  #
        dist = 0.0  # for finding the distance between two minimizers #
                ...
                for j in range(self.GKLS_num_minima):
                    if (i != j):
                        temp_d1 = self.GKLS_norm(
                            self.GKLS_minima.local_min[i],
                            self.GKLS_minima.local_min[j]) - self.GKLS_minima.rho[j]
                        if (temp_d1 < temp_min):
                            temp_min = temp_d1
    ...
    # Fix to 0 the peak(i) values of the paraboloid and the global min  #
        ...
        for i in range(2, self.GKLS_num_minima):
            ...
            temp_min = (self.GKLS_minima.rho[i] - temp_d1) * \
                (self.GKLS_minima.rho[i] - temp_d1) + self.GKLS_minima.f[0]  # the conditional minimum at the boundary#
    ...
    # *******************************************************************#
        self.GKLS_glob.num_global_minima = 0
        for i in range(self.GKLS_num_minima):
            if ((self.GKLS_minima.f[i] >= self.GKLS_global_value - GKLSFunction.GKLS_PRECISION)
                    and (self.GKLS_minima.f[i] <= self.GKLS_global_value + GKLSFunction.GKLS_PRECISION)):
    ...
    def GKLS_domain_alloc(self) -> int:
    ...
    def SetDimension(self, value: int) -> None:
    ...
    def GetOptimumPoint(self) -> List[float]:
        if (self.isArgSet == 1):
            return self.GKLS_minima.local_min[self.GKLS_glob.gm_index[0]]
        raise RuntimeError("isArgSet is not 1")

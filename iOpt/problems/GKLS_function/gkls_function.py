...
class GKLSFunction:
    ...
    def CalculateDFunction(self, x:np.ndarray(shape = (1), dtype = np.double)):
        ...
        if (self.isArgSet == False): 
            return GKLSFunction.GKLS_MAX_VALUE

        for i in range(self.GKLS_dim): 
            if ((x[i] < self.GKLS_domain_left[i] - GKLSFunction.GKLS_PRECISION) or (x[i] > self.GKLS_domain_right[i] + GKLSFunction.GKLS_PRECISION)):
                return  GKLSFunction.GKLS_MAX_VALUE
  # Check wether x belongs to some basin of local minima, M(index) <> T *#
  # Attention: number of local minima must be >= 2 *#
        index = 1
        while ((index < self.GKLS_num_minima) and (self.GKLS_norm(self.GKLS_minima.local_min[index], x) > self.GKLS_minima.rho[index])):
            index = index + 1
  ...
  # Return the value of the cubic interpolation function *#
    ...
    def SetFunctionNumber(self, number):
        ...
        if (self.mIsGeneratorMemoryAllocated == True):
            self.GKLS_free()
  ...
  # the paraboloid minimum value #
        for i in range(self.GKLS_dim): 
            self.GKLS_minima.local_min[0][i] = self.GKLS_domain_left[i] + self.rnd_num[self.rnd_counter] * (self.GKLS_domain_right[i] - self.GKLS_domain_left[i])
            self.rnd_counter+=1
        ...
        self.GKLS_minima.local_min[1][0] = self.GKLS_minima.local_min[0][0] + self.GKLS_global_dist * np.cos(GKLSFunction.PI * self.rnd_num[self.rnd_counter])
        if ((self.GKLS_minima.local_min[1][0] > self.GKLS_domain_right[0] - GKLSFunction.GKLS_PRECISION) or (self.GKLS_minima.local_min[1][0] < self.GKLS_domain_left[0] + GKLSFunction.GKLS_PRECISION)):
            self.GKLS_minima.local_min[1][0] = self.GKLS_minima.local_min[0][0] - self.GKLS_global_dist * np.cos(GKLSFunction.PI * self.rnd_num[self.rnd_counter])
    ...
    # the coordinates x(i), i=1,...,GKLS_dim-2 (not last!) #
        for j in range(1, self.GKLS_dim - 1): 
            self.GKLS_minima.local_min[1][j] = self.GKLS_minima.local_min[0][j] + self.GKLS_global_dist * np.cos(2.0 * GKLSFunction.PI * self.rnd_num[self.rnd_counter]) * sin_phi
            if ((self.GKLS_minima.local_min[1][j] > self.GKLS_domain_right[j] - GKLSFunction.GKLS_PRECISION) or (self.GKLS_minima.local_min[1][j] < self.GKLS_domain_left[j] + GKLSFunction.GKLS_PRECISION)):
                self.GKLS_minima.local_min[1][j] = self.GKLS_minima.local_min[0][j] - self.GKLS_global_dist * np.cos(2.0 * GKLSFunction.PI * self.rnd_num[self.rnd_counter]) * sin_phi
    ...
    # Generate the last coordinate x(GKLS_dim-1) #
        self.GKLS_minima.local_min[1][self.GKLS_dim - 1] = self.GKLS_minima.local_min[0][self.GKLS_dim - 1] + self.GKLS_global_dist * sin_phi
        if ((self.GKLS_minima.local_min[1][self.GKLS_dim - 1] > self.GKLS_domain_right[self.GKLS_dim - 1] - GKLSFunction.GKLS_PRECISION) or (self.GKLS_minima.local_min[1][self.GKLS_dim - 1] < self.GKLS_domain_left[self.GKLS_dim - 1] + GKLSFunction.GKLS_PRECISION)):
            self.GKLS_minima.local_min[1][self.GKLS_dim - 1] = self.GKLS_minima.local_min[0][self.GKLS_dim - 1] - self.GKLS_global_dist * sin_phi
  ...
  # region of the global minimizer M(i) #
            ...
            while (i < self.GKLS_num_minima):
                while (True):
                    ...
                    for j in range(self.GKLS_dim):                    
                        self.GKLS_minima.local_min[i][j] = self.GKLS_domain_left[j] + self.rnd_num[self.rnd_counter] * (self.GKLS_domain_right[j] - self.GKLS_domain_left[j])
                        self.rnd_counter+=1
                        if (self.rnd_counter == GKLSRandomGenerator.NUM_RND):                        
                            self.mRndGenerator.GenerateNextNumbers() #ranf_array(rnd_num, NUM_RND);
                            self.rnd_counter = 0

                    if ((self.GKLS_global_radius + gap) - self.GKLS_norm(self.GKLS_minima.local_min[i], self.GKLS_minima.local_min[1]) > GKLSFunction.GKLS_PRECISION) == False:
                        break
                i+=1
            error = self.GKLS_coincidence_check()
            if ((error == GKLSFunction.GKLS_PARABOLA_MIN_COINCIDENCE_ERROR) or (error == GKLSFunction.GKLS_LOCAL_MIN_COINCIDENCE_ERROR)) == False:
                break
    ...
    def GKLS_parameters_check(self):
        ...
        for i in range(self.GKLS_dim):  
            ...
            if (tmp < min_side):
                min_side = tmp
        if ((self.GKLS_global_dist >= 0.5 * min_side - GKLSFunction.GKLS_PRECISION) or (self.GKLS_global_dist <= GKLSFunction.GKLS_PRECISION)):
            return GKLSFunction.GKLS_GLOBAL_DIST_ERROR # global distance error #
        if ((self.GKLS_global_radius >= 0.5 * self.GKLS_global_dist + GKLSFunction.GKLS_PRECISION) or (self.GKLS_global_radius <= GKLSFunction.GKLS_PRECISION)):
  ...
  # seed number between 0 and 2^30-3 = 1,073,741,821#
    ...
    def GKLS_coincidence_check(self):
        ...
        for i in range(2, self.GKLS_num_minima):
            if (self.GKLS_norm(self.GKLS_minima.local_min[i], self.GKLS_minima.local_min[0]) < GKLSFunction.GKLS_PRECISION):
        ...
        for i in range(1, self.GKLS_num_minima-1):
            for i in range(i+1, self.GKLS_num_minima):
                if (self.GKLS_norm(self.GKLS_minima.local_min[i], self.GKLS_minima.local_min[j]) < GKLSFunction.GKLS_PRECISION):
  ...
  # paraboloid vertex.                                                           #
        ...
        for i in range(2, self.GKLS_num_minima):
            dist = (self.GKLS_norm(self.GKLS_minima.local_min[i], self.GKLS_minima.local_min[1]) - self.GKLS_global_radius - GKLSFunction.GKLS_PRECISION)
  ...
  # do not overlap                                                           #
        for i in range(self.GKLS_num_minima):
            ...
            # rho(i) := max {rho(i),min[||M(i)-M(j)|| - rho(j): i !=j]},      #
                ...
                for j in range(self.GKLS_num_minima):
                    if (i != j):
                        temp_d1 = self.GKLS_norm(self.GKLS_minima.local_min[i], self.GKLS_minima.local_min[j]) - self.GKLS_minima.rho[j]
                        if (temp_d1 < temp_min):
                            temp_min = temp_d1;
  ...
  # Fix to 0 the peak(i) values of the paraboloid and the global min  #
        self.GKLS_minima.peak[0] = 0.0; self.GKLS_minima.peak[1] = 0.0;
        for i in range(2, self.GKLS_num_minima): 
            ...
            temp_min = (self.GKLS_minima.rho[i] - temp_d1)*(self.GKLS_minima.rho[i] - temp_d1) + self.GKLS_minima.f[0]; #the conditional minimum at the boundary#
  ...
  #*******************************************************************#
        self.GKLS_glob.num_global_minima = 0;
        for i in range(self.GKLS_num_minima):
            if ((self.GKLS_minima.f[i] >= self.GKLS_global_value - GKLSFunction.GKLS_PRECISION) and (self.GKLS_minima.f[i] <= self.GKLS_global_value + GKLSFunction.GKLS_PRECISION)):
    ...
    def SetDimension(self, value):
      #assert(value > 1 && value <= NUM_RND);
        ...
        self.GKLS_dim = value;
        if (self.mIsDomainMemeoryAllocated == True):
            self.GKLS_domain_free();

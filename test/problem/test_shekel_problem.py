...
class TestShekel(unittest.TestCase):
    ...
    def test_Calculate(self):
        ...
        cShekel = np.array( [ 1.11196, 1.11025, 1.19145, 1.13756, 1.17645, 1.18894, 1.18148, 1.16138, 1.10642, 1.04509],
                            dtype=np.double)
        kShekel = np.array([ 23.93246, 17.63833, 18.27250, 19.15204, 14.05667, 9.04614, 17.96243, 20.56200, 21.70278, 10.51653],
                           dtype=np.double)
        NUM_SHEKEL_COEFF = 10
        res: np.double = 0
        for i in range(NUM_SHEKEL_COEFF):
            res = res + (
                        2 * kShekel[i] * (point.floatVariables[0] - aShekel[i])) / \
                  pow((kShekel[i] * pow(point.floatVariables[0] - aShekel[i],
                                                           2.0) + cShekel[i]), 2)

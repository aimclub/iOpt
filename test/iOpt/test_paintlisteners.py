...
from iOpt.method.listener import (
    AnimationNDPaintListener,
    AnimationPaintListener,
    ConsoleFullOutputListener,
    StaticNDPaintListener,
    StaticPaintListener,
)
...
class TestStaticPaintRastrigin(unittest.TestCase):
    def setUp(self):
        ...
        pl = StaticPaintListener("output", "rastrigin_1_3.5_0.001.png", 0)
...
class TestAnimatePaintRastriginWithoutOF(unittest.TestCase):
    def setUp(self):
        ...
        params = SolverParameters(r=3.5, eps=0.001)
        self.solver = Solver(self.problem, parameters=params)
        apl = AnimationPaintListener("output", "rastrigin_1_3.5_0.001_anim_wof.png",
                                     isPointsAtBottom=False, toPaintObjFunc=False)
        self.solver.AddListener(apl)
        spl = StaticPaintListener("output", "rastrigin_1_3.5_0.001_stat_wof.png",
                                  0, isPointsAtBottom=False, toPaintObjFunc=False)
...
class TestAnimatePaintXSquared2DWithoutOF(unittest.TestCase):
    def setUp(self):
        ...
        apl = AnimationNDPaintListener(
            "output", "xsquared_2_3.0_0.01_anim_wof.png", varsIndxs=[
                0, 1], toPaintObjFunc=False)
        self.solver.AddListener(apl)
        spl = StaticNDPaintListener(
            "output", "xsquared_2_3.0_0.01_stats_wof.png", varsIndxs=[
                0, 1], toPaintObjFunc=False)
        self.solver.AddListener(spl)

...
class mco_test7(Problem):
    ...
    def __init__(self):
        ...
        self.lower_bound_of_float_variables.fill(-4)
    ...
    def calculate(self, point: Point, function_value: FunctionValue) -> FunctionValue:
        ...
        if function_value.functionID == 0:  # OBJECTIV 1
            val = 0
            for i in range(3):
                val += (x[i] - 1 / math.sqrt(3)) * (x[i] - 1 / math.sqrt(3))
            result = np.double(1 - math.exp(-val))
